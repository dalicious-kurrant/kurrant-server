...
public class DeliveryServiceImpl implements DeliveryService {
    ...
    public DeliveryDto getDeliverySchedule(String start, String end, List<BigInteger> groupIds, List<BigInteger> spotIds, Integer isAll) {
        ...
        if(orderItemDailyFoods.isEmpty() && deliveryInstanceList.isEmpty()) return DeliveryDto.create(groupAllList, deliveryInfoList, spotAllList);
        else if(deliveryInstanceList.isEmpty()) {
            // service date 묶기
            ...
            for (ServiceDateDto serviceDateDto : serviceDateDtoMap.keySet()) {
                ...
                Map<Spot, List<OrderItemDailyFood>> spotDailyFoodMap = serviceDateDtoMap.get(serviceDateDto).stream()
                ...
                deliveryInfoList = spotDailyFoodMap.entrySet().stream()
                        .map(entry -> {
                            ...
                            return deliveryMapper.toDeliveryInfo(serviceDateDto.getServiceDate(), Collections.singletonList(deliveryGroup));
                        })
                        .collect(Collectors.toList());
