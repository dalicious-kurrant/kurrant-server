...
public class UserServiceImpl implements UserService {
  ...
  @Override
  @Transactional
  public void changePhoneNumber(HttpServletRequest httpServletRequest,
      ChangePhoneRequestDto changePhoneRequestDto) throws UnsupportedEncodingException,
    ...
    // 인증번호가 일치하는지 확인한다.
    verifyUtil.verifyCertificationNumber(changePhoneRequestDto.getKey(),
        RequiredAuth.MYPAGE_CHANGE_PHONE_NUMBER);
    // 비밀번호를 업데이트 한다.
  ...
  public void changePassword(HttpServletRequest httpServletRequest,
    ...
    User user = commonService.getUser(httpServletRequest);
    // 현재 비밀번호가 일치하는지 확인
    if (!passwordEncoder.matches(changePasswordRequestDto.getCurrantPassword(),
        user.getPassword())) {
    ...
    if (!changePasswordRequestDto.getNewPassword()
        .equals(changePasswordRequestDto.getNewPasswordCheck())) {
      throw new ApiException(ExceptionEnum.PASSWORD_DOES_NOT_MATCH);
    }
    // 비밀번호 변경
  ...
  @Override
  @Transactional
  public void setEmailAndPassword(HttpServletRequest httpServletRequest,
      SetEmailAndPasswordDto setEmailAndPasswordDto) {
    // 로그인한 유저의 정보를 받아온다.
    User user = commonService.getUser(httpServletRequest);
    ...
    ProviderEmail providerEmail =
        ProviderEmail.builder().provider(Provider.GENERAL).email(email).user(user).build();
    providerEmailRepository.save(providerEmail);
  }

  @Override
  @Transactional
  public ChangeMarketingDto changeAlarmSetting(HttpServletRequest httpServletRequest,
      Boolean isMarketingInfoAgree, Boolean isMarketingAlarmAgree, Boolean isOrderAlarmAgree) {
    // 유저 정보 가져오기
    User user = commonService.getUser(httpServletRequest);
    ...
    if (isMarketingInfoAgree != null) {
      user.changeMarketingAgreement(now, isMarketingInfoAgree, isMarketingInfoAgree,
    ...
    if (isMarketingAlarmAgree != null) {
      ...
      // 주문 알림이 활성화 되어 있지 않을 경우
        ...
        user.changeMarketingAgreement(now, !currantMarketingInfoAgree, isMarketingAlarmAgree,
            currantOrderAlarmAgree);
      }
    }
    // 주문 알림 동의/철회
    if (isOrderAlarmAgree != null) {
      ...
      // 혜택 및 소식 알림이 활성화 되어 있지 않을 경우
        ...
        user.changeMarketingAgreement(now, !currantMarketingInfoAgree, currantMarketingAlarmAgree,
            isOrderAlarmAgree);
      }
    }
    return ChangeMarketingDto.builder().marketingAgree(user.getMarketingAgree())
        .marketingAgreedDateTime(user.getMarketingAgreedDateTime())
        .marketingAlarm(user.getMarketingAlarm()).orderAlarm(user.getOrderAlarm()).build();
  }
  ...
  public OrderDetailDto findOrderByServiceDate(Date startDate, Date endDate) {
    ...
    List<OrderDetail> byServiceDateBetween =
        orderDetailRepository.findByServiceDateBetween(startDate, endDate);
    ...
    byServiceDateBetween.forEach(x -> {
      ...
      OrderItemDto orderItemDto = OrderItemDto.builder().name(food.getName())
          .diningType(x.getEDiningType()).img(food.getImg()).count(x.getCount()).build();
