...
import co.kurrant.app.public_api.dto.user.FindIdRequestDto;
import co.kurrant.app.public_api.dto.user.FindIdResponseDto;
import co.kurrant.app.public_api.dto.user.FindPasswordEmailRequestDto;
import co.kurrant.app.public_api.dto.user.FindPasswordPhoneRequestDto;
import co.kurrant.app.public_api.dto.user.FindPasswordUserCheckRequestDto;
...
public class AuthServiceImpl implements AuthService {
  ...
  public void mailConfirm(MailMessageDto mailMessageDto, String type) throws Exception {
    ...
    switch (requiredAuth) {
      ...
      case FIND_PASSWORD:
        // 존재하는 유저인지 확인
        User user = userRepository.findByEmail(mailMessageDto.getReceivers().get(0))
    ...
    // Redis에 인증번호 저장
    CertificationHash certificationHash = CertificationHash.builder().id(null).type(type)
        .to(receiver).certificationNumber(key).build();
    certificationHashRepository.save(certificationHash);
  }

  // Sms 인증
  @Override
  public void sendSms(SmsMessageRequestDto smsMessageRequestDto, String type)
    ...
    switch (requiredAuth) {
      ...
      case FIND_ID, FIND_PASSWORD:
        // 유저가 존재하는지 확인
        User user = userRepository.findByPhone(smsMessageRequestDto.getTo())
            .orElseThrow(() -> new ApiException(ExceptionEnum.USER_NOT_FOUND));
        break;
    }
    ...
    // Redis에 인증번호 저장
    CertificationHash certificationHash = CertificationHash.builder().id(null).type(type)
        .to(smsMessageRequestDto.getTo()).certificationNumber(key).build();
    certificationHashRepository.save(certificationHash);
  }
  ...
  public User signUp(SignUpRequestDto signUpRequestDto) {
    ...
    if (user == null) {
      UserDto userDto =
          UserDto.builder().email(signUpRequestDto.getEmail()).phone(signUpRequestDto.getPhone())
              .password(hashedPassword).name(signUpRequestDto.getName()).role(Role.USER).build();
    ...
    ProviderEmail providerEmail =
        ProviderEmail.builder().email(mail).provider(Provider.GENERAL).user(user).build();
  ...
  public LoginResponseDto login(LoginRequestDto dto) {
    ...
    JwtTokenProvider.TokenResponseDto token =
        jwtTokenProvider.createToken(user.getId().toString(), arr, user.getName(), null);

    // 로그인 날짜 업데이트
    Timestamp timestamp = Timestamp.valueOf(LocalDateTime.now());
    user.updateRecentLoginDateTime(timestamp);

    return LoginResponseDto.builder().accessToken(token.getAccessToken()).expiresIn(86400).build();
  }
  ...
  public FindIdResponseDto findUserEmail(FindIdRequestDto findIdRequestDto) {
    ...
    // 유저 가져오기
    User user = userRepository.findByPhone(findIdRequestDto.getPhone())
        .orElseThrow(() -> new ApiException(ExceptionEnum.USER_NOT_FOUND));
    ...
    return FindIdResponseDto.builder().connectedSns(connectedSns).email(user.getEmail())
        .recentLoginDateTime(DateUtils.toISO(user.getRecentLoginDateTime())).build();
  }

  @Override
  public void checkUser(FindPasswordUserCheckRequestDto findPasswordUserCheckRequestDto) {
    userRepository
        .findByNameAndEmail(findPasswordUserCheckRequestDto.getName(),
            findPasswordUserCheckRequestDto.getEmail())
        .orElseThrow(() -> new ApiException(ExceptionEnum.USER_NOT_FOUND));
  }
  ...
  public void findPasswordEmail(FindPasswordEmailRequestDto findPasswordEmailRequestDto) {
    ...
    // 유저 정보 가져오기
    User user = userRepository.findByEmail(findPasswordEmailRequestDto.getEmail())
        .orElseThrow(() -> new ApiException(ExceptionEnum.USER_NOT_FOUND));
    // 비밀번호 변경
    String hashedPassword = passwordEncoder.encode(password);
  ...
  public void findPasswordPhone(FindPasswordPhoneRequestDto findPasswordPhoneRequestDto) {
    ...
    userValidator.isValidPassword(password);
    // 유저 정보 가져오기
    User user = userRepository.findByPhone(findPasswordPhoneRequestDto.getPhone())
        .orElseThrow(() -> new ApiException(ExceptionEnum.USER_NOT_FOUND));
    // 비밀번호 변경
    String hashedPassword = passwordEncoder.encode(password);
    user.changePassword(hashedPassword);
